...
void memset_vect(vector<unsigned char>& vect, int value, int begin, int size=0)
{
	int end = begin + size;
	if (size == 0)
	{
...
void memcpy_vect(vector<unsigned char>& des_vect, const vector<unsigned char>& src_vect, int size=0)
{
	if (size==0)
	{
		size = src_vect.size();
...
void gen_sub_keys(const vector<unsigned char>& main_key, vector<KeySet>& sub_keys)
{
	int i, j;
	int shift_size;
	...
	for (i = 0; i < 56; i++) {
		...
		sub_keys[0].k[i / 8] |= (shift_byte >> i % 8);
	}

	for (i = 0; i < 3; i++) {
		sub_keys[0].c[i] = sub_keys[0].k[i];
	}

	sub_keys[0].c[3] = sub_keys[0].k[3] & 0xF0;

	for (i = 0; i < 3; i++) {
		sub_keys[0].d[i] = (sub_keys[0].k[i + 3] & 0x0F) << 4;
		sub_keys[0].d[i] |= (sub_keys[0].k[i + 4] & 0xF0) >> 4;
	}

	sub_keys[0].d[3] = (sub_keys[0].k[6] & 0x0F) << 4;


	for (i = 1; i < 17; i++) {
		for (j = 0; j < 4; j++) {
			sub_keys[i].c[j] = sub_keys[i - 1].c[j];
			sub_keys[i].d[j] = sub_keys[i - 1].d[j];
		}
		...
		// Process C
		first_shift_bits = shift_byte & sub_keys[i].c[0];
		second_shift_bits = shift_byte & sub_keys[i].c[1];
		third_shift_bits = shift_byte & sub_keys[i].c[2];
		fourth_shift_bits = shift_byte & sub_keys[i].c[3];

		sub_keys[i].c[0] <<= shift_size;
		sub_keys[i].c[0] |= (second_shift_bits >> (8 - shift_size));

		sub_keys[i].c[1] <<= shift_size;
		sub_keys[i].c[1] |= (third_shift_bits >> (8 - shift_size));

		sub_keys[i].c[2] <<= shift_size;
		sub_keys[i].c[2] |= (fourth_shift_bits >> (8 - shift_size));

		sub_keys[i].c[3] <<= shift_size;
		sub_keys[i].c[3] |= (first_shift_bits >> (4 - shift_size));

		// Process D
		first_shift_bits = shift_byte & sub_keys[i].d[0];
		second_shift_bits = shift_byte & sub_keys[i].d[1];
		third_shift_bits = shift_byte & sub_keys[i].d[2];
		fourth_shift_bits = shift_byte & sub_keys[i].d[3];

		sub_keys[i].d[0] <<= shift_size;
		sub_keys[i].d[0] |= (second_shift_bits >> (8 - shift_size));

		sub_keys[i].d[1] <<= shift_size;
		sub_keys[i].d[1] |= (third_shift_bits >> (8 - shift_size));

		sub_keys[i].d[2] <<= shift_size;
		sub_keys[i].d[2] |= (fourth_shift_bits >> (8 - shift_size));

		sub_keys[i].d[3] <<= shift_size;
		sub_keys[i].d[3] |= (first_shift_bits >> (4 - shift_size));

		for (j = 0; j < 48; j++) {
			shift_size = sub_key_permutation[j];
			if (shift_size <= 28) {
				shift_byte = 0x80 >> ((shift_size - 1) % 8);
				shift_byte &= sub_keys[i].c[(shift_size - 1) / 8];
				shift_byte <<= ((shift_size - 1) % 8);
			}
			else {
				shift_byte = 0x80 >> ((shift_size - 29) % 8);
				shift_byte &= sub_keys[i].d[(shift_size - 29) / 8];
				shift_byte <<= ((shift_size - 29) % 8);
			}

			sub_keys[i].k[j / 8] |= (shift_byte >> j % 8);
		}
	}
}

void process_message(vector<unsigned char>& in_data_piece, const vector<KeySet>& sub_keys, OptType opt_type, vector<unsigned char>& out_data_piece)
{
	int i, k;
	int shift_size;
	...
	memset_vect(out_data_piece, 0, 0);

	for (i = 0; i < 64; i++) {
		shift_size = initial_message_permutation[i];
		shift_byte = 0x80 >> ((shift_size - 1) % 8);
		shift_byte &= in_data_piece[(shift_size - 1) / 8];
		shift_byte <<= ((shift_size - 1) % 8);
	...
	for (i = 0; i < 64; i++) {
		...
		out_data_piece[i / 8] |= (shift_byte >> i % 8);
	}
}

// mode of opt is CBC
void do_crypt(const vector<unsigned char>& in_data, OptType opt_type, const vector<KeySet>& sub_keys, vector<unsigned char>& out_data)
{
	out_data.clear();

	int block_count = 0;
	int number_of_blocks = in_data.size() / 8 + (in_data.size() % 8 ? 1 : 0);
	vector<unsigned char> in_data_piece(8, 0);
	vector<unsigned char> out_data_piece(8, 0);
	vector<unsigned char> tmp_data_piece(8, 0);
	vector<unsigned char> init_vector = { 1,2,3,4,5,6,7,8 };
	while (true)
	{
		sub_vect(in_data, block_count * 8, 8, in_data_piece);
		block_count++;
		if (block_count == number_of_blocks)
		{
			if (opt_type == OptType::kEncrypt)
			{
				int padding = 8 - in_data.size() % 8;
				if (padding < 8)
				{
					memset_vect(in_data_piece, padding, 8 - padding, padding);
				}

				vect_xor(in_data_piece, init_vector);
				process_message(in_data_piece, sub_keys, opt_type, out_data_piece);
...
{
	vector<unsigned char> main_key(8);
	gen_key(main_key);
	print_vect(main_key, "main_key");

	vector<KeySet> sub_keys(17);
	gen_sub_keys(main_key, sub_keys);

	string plain_text = "my name is wolfand11!";
	...
	do_crypt(plain_data, OptType::kEncrypt, sub_keys, cipher_data);
	print_vect(cipher_data, "cipher_data");

	vector<unsigned char> plain_data_from_cipher;
	do_crypt(cipher_data, OptType::kDecrypt, sub_keys, plain_data_from_cipher);
